...
class AudioAnim(ledworks.Animation):
	def setup(self, player):
		self.mapper = ledworks.log_filterbank(
			rate=player.rate,
			bins=self.n // 2,
			n_fft=player.window_size // 2,
			f_min=20,
			f_max=320,
		)

		self.normalize = ledworks.filter.Normalize(player.fps)
		self.sustain = ledworks.filter.Sustain(0.08, player.fps)
...
class KickReactive(ledworks.Animation):
	def setup(self, player):
		...
		self.normalize = ledworks.filter.Normalize(player.fps)
		self.sustain = ledworks.filter.Sustain(0.1, player.fps)
	...
	@ledworks.cycle_all(4.0)
	def test(self, idx, delta, elapsed):
		self.assign(idx, ledworks.gen.abrupt, duration=1.0)
		self.assign((idx + self.n // 2) % self.n, ledworks.gen.abrupt, duration=1.0)
...
player = ledworks.Player(N, view=view, stream=ledworks.stream.get_stream(5), fps=60)
